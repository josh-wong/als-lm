...
def check_ollama_running(ollama_url, timeout=10):
    ...
    Returns a list of available model names, or exits on failure.
    """
...
def generate_chat_response(ollama_url, model_name, system_prompt, user_message,
                           max_tokens, timeout, num_ctx):
    """Generate a single response via the Ollama /api/chat endpoint.

    Identical to the baseline version except for the addition of num_ctx in
    the options dict to prevent silent context window truncation when the
    augmented prompt exceeds the default 2048-token window.

    Returns (response_text, tokens_generated) on success, or
...
def run_eval_stages(output_dir, benchmark_path, registry_path, config_name):
    """Invoke evaluation stages 2-6 via subprocess.

    Each stage is a separate Python script invoked with explicit file path
    arguments. The report filename is per-config to distinguish from the
    baseline report.

    Returns True if all stages succeeded, False otherwise.
    """
    paths = {
        ...
        "report": os.path.join(output_dir, f"hallucination_eval_{config_name}.md"),
    }
