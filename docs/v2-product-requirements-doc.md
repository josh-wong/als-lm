...
- RAG re-comparison using the best ALS-LM-1 configuration only (PubMedBERT 500-token)
...
- **Full RAG re-run (all 4 configurations):** Only the best-performing ALS-LM-1 configuration (PubMedBERT 500-token) is re-run. The ALS-LM-1 investigation already established the full RAG comparison across all configurations.
- **Web-based or graphical user interfaces:** The evaluation interface remains a command-line demo, consistent with ALS-LM-1 scope.
...
- **Non-English content:** The model trains on English-language content only, consistent with ALS-LM-1 scope.
...
| FR-1.2 | Identify and evaluate new ALS-relevant sources beyond the three categories used in ALS-LM-1.                                     | Must have   |
| FR-1.3 | Implement scrapers for approved new sources following the ALS-LM-1 data collection patterns.                                     | Must have   |
| FR-1.4 | Validate corpus quality through statistical comparison with the ALS-LM-1 corpus (size, source distribution, deduplication rate). | Must have   |
...
| FR-2.4 | Implement checkpoint saving and resumption consistent with ALS-LM-1 practices.                             | Must have   |
...
| FR-4.2 | Run the hallucination benchmark on the instruction-tuned model and report results in the same format as ALS-LM-1.               | Must have   |
...
FR-5 covers the release deliverables for ALS-LM-2, ensuring that all documentation, model artifacts, and project metadata are updated and published. This group extends [ALS-LM-1 FR-5 (model export and Ollama integration)](v1-product-requirements-doc.md) and [ALS-LM-1 FR-6 (CLI demo)](v1-product-requirements-doc.md).

| ID     | Requirement                                                                                             | Priority    |
|--------|---------------------------------------------------------------------------------------------------------|-------------|
| FR-5.1 | Write an updated research paper documenting ALS-LM-2 methodology, findings, and cross-version analysis. | Must have   |
...
Non-functional requirements address hardware, reproducibility, documentation, and ALS-LM-2-specific quality constraints. The first three NFR groups carry forward from the [ALS-LM-1 product requirements document](v1-product-requirements-doc.md) with ALS-LM-2 extensions.
...
| NFR-2.3 | A `requirements.txt` or equivalent locks all dependency versions including any new ALS-LM-2 dependencies. | Must have   |
...
| ID      | Requirement                                                                                               | Priority    |
|---------|-----------------------------------------------------------------------------------------------------------|-------------|
| NFR-3.1 | README provides setup instructions covering both ALS-LM-1 and ALS-LM-2 components.                        | Must have   |
| NFR-3.2 | All data sources (ALS-LM-1 and new ALS-LM-2 sources) are inventoried with license and access information. | Must have   |
...
| ID      | Requirement                                                                                                                       | Priority    |
|---------|-----------------------------------------------------------------------------------------------------------------------------------|-------------|
| NFR-5.1 | The evaluation benchmark preserves the same 160 questions and key facts used in ALS-LM-1.                                         | Must have   |
...
- The updated research paper is complete and documents ALS-LM-2 methodology, findings, and cross-version analysis with honest evaluation of results regardless of accuracy outcomes.
...
- The evaluation benchmark is re-run on the instruction-tuned model with results presented alongside ALS-LM-1 results in a single comparison.
- All ALS-LM-2 milestones (v1.2.0 through v2.0.0) are shipped with corresponding documentation.
...
The ALS-LM-2 investigation is divided into five milestones executed sequentially. Each milestone depends on the prior milestone's completion, as the pipeline is cumulative: improved data feeds into model training, which feeds into instruction tuning, which feeds into evaluation.
...
The following risks were identified during planning, combining risks that carry forward from [v1](v1-product-requirements-doc.md) with new risks specific to the ALS-LM-2 investigation.

| Risk                                                            | Likelihood | Impact | Mitigation                                                                                                                                                       |
|-----------------------------------------------------------------|------------|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1B training exceeds VRAM even with ZeRO Stage 2 CPU offloading. | Low        | High   | ALS-LM-1 516M model used 6.37 GB peak VRAM, leaving headroom. ZeRO Stage 3 is available as fallback.                                                             |
...
The project relies on the following external tools and services. Continuing ALS-LM-1 dependencies are listed alongside new ALS-LM-2-specific additions.

| Dependency                   | Purpose                                             | Version/notes                                                        |
|------------------------------|-----------------------------------------------------|----------------------------------------------------------------------|
| PubMed Central API           | Research paper collection (v1, extended in v2)      | Continuing from v1; expanded source queries for ALS-LM-2             |
| ClinicalTrials.gov API       | Clinical trial data (v1, continuing)                | Continuing from ALS-LM-1                                             |
...
| Evaluation harness (updated) | Instruction-formatted prompt evaluation (UPDATED)   | Adapted from ALS-LM-1 harness; preserves cross-version comparability |
