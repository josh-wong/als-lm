...
**Date:** March 23, 2026 (v2 update; original: March 6, 2026)
**Status:** Complete
**Version note:** This v2 update incorporates v1.4.0 SFT failure analysis (Section 8) and v1.5.0 QLoRA domain adaptation (Section 9) experimental results. The original paper covered Sections 1-7 only.

---

## Abstract

Domain-specific language models in biomedicine typically benefit from billions of tokens of broad biomedical literature. We investigate what happens at the opposite extreme: training a purpose-built decoder-only transformer on just 142M tokens of curated amyotrophic lateral sclerosis (ALS) research, a corpus 80x below the Chinchilla-optimal ratio. This places our investigation in a regime where conventional scaling laws predict that the model cannot acquire meaningful factual knowledge, and our results confirm this prediction across six model variants spanning three approach families.

We construct a reproducible pipeline and train a 516M-parameter GPT-2-style model from scratch on consumer hardware (NVIDIA RTX 3060, 12GB VRAM). The model achieves Well-fit convergence (validation gap +0.42%) yet attains only 0.21% mean factual accuracy on our 160-question ALS benchmark, demonstrating that language-modeling competence and factual knowledge acquisition are distinct capabilities with fundamentally different data requirements.

We extend the investigation across six model variants spanning from-scratch training (516M, 1B base, 1B with supervised fine-tuning), pretrained fine-tuning (GPT-2 large 774M), and pretrained domain adaptation (Qwen 2.5 1.5B Instruct baseline and QLoRA). No approach exceeds the RAG baseline accuracy of 14.3%. Scaling parameters without additional data reduces accuracy (1B: 0.00% vs. 500M: 0.21%). Instruction tuning a knowledge-deficient base model produces complete output collapse (160/160 degenerate responses). Domain adaptation of an instruction-capable model with 970 pairs degrades accuracy from 10.31% to 7.24%. The central finding is that data scarcity, not architecture, scaling, or training strategy, is the binding constraint for domain-specific factual knowledge acquisition.

This work makes three contributions: (1) a complete, reproducible, open-source pipeline for domain-specific language model training, evaluation, and comparison; (2) honest documentation of negative results across six approaches, providing empirical data points at the extreme low end of the data-scaling curve; and (3) a reusable evaluation framework consisting of a 160-question curated benchmark, a 5-mode failure taxonomy, and entity-based fabrication detection (~48K entities) applicable to other domain-specific model evaluations.
...
The remainder of this paper is organized as follows. Section 2 surveys related work across domain-specific language models, hallucination evaluation, medical RAG, and data-scaling laws. Section 3 details our methodology, covering the data pipeline, tokenizer, model architecture, and training procedure. Section 4 describes our evaluation framework, including benchmark design, scoring methodology, fabrication detection, and failure taxonomy. Section 5 presents training results and hallucination evaluation findings across three quantization levels. Section 6 reports the RAG comparison experiment and failure decomposition analysis. Section 7 reports a controlled comparison between the from-scratch model and a fine-tuned GPT-2 large, revealing the impact of pretrained knowledge on ALS-specific accuracy. Section 8 reports the results of supervised fine-tuning (SFT) on the from-scratch 1B model, revealing complete output collapse when instruction tuning is applied without a foundation of pretrained knowledge. Section 9 investigates QLoRA domain adaptation of a pretrained instruction-capable model, testing whether domain-specific fine-tuning can improve factual accuracy when applied to a model with existing biomedical knowledge. Section 10 discusses the implications of our findings, with particular attention to the data deficit, embedding-model impact, and the loss-accuracy gap. Section 11 concludes with a synthesis of all six model evaluations, framing data scarcity as the fundamental constraint, presenting hypothesis verdicts, and identifying directions for future work.
...
| ALS-LM (fine-tuned) | 774M | 146M tokens (ALS fine-tuning) | GPT-2 large | 2026 | 3.12% accuracy; 97.5% degenerate output          |
...
The failure mode distribution for ALS-LM (using the Q8_0 results as representative, since all three levels produce equivalent patterns) reveals three dominant categories: confident fabrication at 53 responses (33.1%), degenerate output at 52 responses (32.5%), and plausible blending at 38 responses (23.8%). Outdated information accounts for 17 responses (10.6%). Boundary confusion and accurate but misleading each account for 0 responses, and no responses were classified as accurate.
...
The most striking finding is the degenerate output dominance. The fine-tuned GPT-2 large produces degenerate (repetitive or incoherent) output for 97.5% of evaluation questions (156 out of 160), compared to 32.5% for the from-scratch model (52 out of 160). The from-scratch model exhibits diverse failure modes: confident fabrication accounts for 33.1% of responses (53 out of 160), plausible blending for 23.8% (38 out of 160), and degenerate output for 32.5% (52 out of 160). The fine-tuned model's failures are qualitatively different, dominated almost entirely by degenerate repetition (97.5%) with only 2 instances of confident fabrication and 2 of plausible blending.
...
## 8. Supervised fine-tuning on the from-scratch 1B model

The GPT-2 large comparison in Section 7 revealed that pretrained general knowledge improves accuracy but that the completion-based architecture cannot follow structured prompts. A natural follow-up question arises: can supervised fine-tuning (SFT) on instruction-response pairs teach the from-scratch model to produce structured answers, thereby surfacing whatever factual knowledge it may have internalized during pretraining on 153M ALS tokens? The project's [v2 white paper](v2-white-paper.md) proposed this as a testable hypothesis: that instruction tuning could help the from-scratch model surface internalized ALS knowledge in structured Q&A format. This section reports the results of that experiment, which produced a complete failure that is itself informative about the prerequisites for successful instruction tuning.

### 8.1 Methodology

We constructed an Alpaca-format instruction dataset of 970 validated ALS question-answer pairs, split into 776 training and 194 validation examples. Each pair follows the standard Alpaca instruction-input-output format, with completion-only loss masking applied during training so that the model learns to generate responses without being penalized for the instruction portion of each example.

The SFT procedure was applied to the 1B model trained from scratch on approximately 153M ALS-domain tokens. Training ran for 2 epochs over 290 steps with a learning rate of 2e-5, 50-step warmup, and an effective batch size of 8. Training loss decreased from 9.0 to 4.7, and validation loss reached 4.8782 at the best checkpoint (epoch 1, step 290). We exported the instruction-tuned model to GGUF format and evaluated it against the same 160-question ALS benchmark using the identical 6-stage pipeline described in Section 4, with Q8_0 as the representative quantization level.

### 8.2 Results

The instruction-tuned 1B model achieved 0.00% mean accuracy with 160/160 responses classified as degenerate. Table 7 compares the 1B base model and the 1B instruction-tuned model on the key evaluation metrics.

**Table 7.** 1B base vs. 1B instruction-tuned model on the 160-question ALS benchmark (Q8_0 quantization level). Instruction tuning destroyed the base model's limited output diversity without producing any structured responses.

| Metric                   | 1B base (from-scratch) | 1B instruction-tuned (SFT) |
|--------------------------|:----------------------:|:--------------------------:|
| Mean accuracy            |                  0.00% |                      0.00% |
| Coherent responses       |        56/160 (35.0%)  |            0/160 (0.0%)    |
| Degenerate responses     |       104/160 (65.0%)  |          160/160 (100.0%)  |
| Fabrication rate         |                100.0%  |                      0.0%  |
| Capability gap           |                 35.0%  |                      0.0%  |

The SFT process produced complete output collapse. Every response across all 160 benchmark questions consists of the same degenerate pattern: concatenated repetition of the word "The." A representative example illustrates the failure mode:

> **Question:** What are the primary genetic mutations associated with familial ALS?
> **Response:** TheTheTheTheTheTheTheTheTheTheTheTheTheTheTheThe...

The word "The" is the most common English word and the most frequent token in the training corpus. The model has collapsed to a maximum-likelihood single-token distribution, repeating the highest-probability token indefinitely regardless of the input question. This pattern is consistent across all 8 ALS knowledge categories, all difficulty levels, and all question types.

The base 1B model without SFT, while also achieving 0.00% accuracy, at least produces 56/160 non-degenerate responses (35.0% coherence). These non-degenerate responses include diverse failure modes: 19 classified as confident fabrication, 29 as plausible blending, and 8 as outdated information. The SFT process destroyed this diversity entirely rather than redirecting it toward accurate answers.

### 8.3 Root cause analysis

Three compounding factors explain the complete failure of instruction tuning on the from-scratch 1B model.

**Data scale.** The 970 instruction pairs are one to two orders of magnitude below published SFT dataset sizes. Stanford Alpaca used 52,000 instruction pairs to fine-tune LLaMA 7B. Databricks Dolly used 15,000 human-written pairs. Even LIMA, one of the smallest published SFT datasets at approximately 1,000 examples, relied on a pretrained 65B parameter model with extensive general knowledge as its foundation. The combination of 970 pairs with a model that has no general language understanding is far below the minimum viable threshold for instruction tuning.

**Base model quality.** The 1B model was trained from scratch on approximately 153M domain-specific tokens. It has no general English understanding, no exposure to WebText or any broad internet corpus, and no pre-existing ability to follow instructions. Pretrained models like GPT-2 or LLaMA bring billions of tokens of general knowledge that SFT can redirect toward specific tasks. Without this foundation, SFT must simultaneously teach language understanding, task format, and domain knowledge from only 970 examples, a task that is not feasible.

**Compounding knowledge deficit.** The base model's 0.00% benchmark accuracy indicates not merely an instruction-following deficit but a complete knowledge deficit. There is no internalized factual knowledge for SFT to surface. The project's [v2 white paper](v2-white-paper.md) hypothesized that the model had learned factual content during pretraining that it simply could not express in Q&A format. The 4-model comparison across Sections 5 through 7 disproves this: the 1B base model produces diverse but universally inaccurate responses, indicating it learned statistical patterns in ALS text but not extractable medical facts. SFT cannot create knowledge that does not exist in the base model.

The training loss reduction from 9.0 to 4.7 reinforces this diagnosis. The model did optimize the SFT loss objective, but the "learning" it achieved was convergence to a degenerate single-token distribution that minimizes cross-entropy by concentrating probability mass on the most common token. Loss curves alone cannot distinguish between meaningful learning and degenerate convergence.

### 8.4 Limitations

Three limitations constrain the interpretation of this SFT result.

**Single SFT configuration.** Only one hyperparameter configuration was tested (learning rate 2e-5, 2 epochs, effective batch size 8). Alternative learning rates, longer training, or different warmup schedules might produce different results, though the fundamental knowledge deficit of the base model would remain.

**Small instruction dataset.** The 970 instruction pairs are small even relative to minimal SFT datasets in the literature. A larger instruction dataset (5,000-50,000 pairs) might cross the threshold needed to prevent degenerate collapse, though without pre-existing knowledge in the base model, it is unclear whether more instruction data alone would be sufficient.

**No comparison with larger instruction datasets on the same base.** We did not test the effect of instruction dataset size on the from-scratch base model. Such an ablation would isolate the data-scale factor from the base-model-quality factor, but generating thousands of additional validated ALS instruction pairs was beyond the scope of this experiment.

SFT cannot create knowledge that does not exist in the base model. This motivates a different approach: rather than attempting to teach question-answering behavior to a model that lacks foundational knowledge, we can start from a pretrained instruction-capable model that already possesses general biomedical knowledge and adapt it toward the ALS domain.

## 9. QLoRA domain adaptation of a pretrained instruction model

The SFT failure in Section 8 confirmed that instruction tuning requires a foundation of existing knowledge: a model trained from scratch on 153M domain-specific tokens has no internalized factual knowledge for SFT to surface, regardless of instruction dataset quality. This finding motivated a fundamentally different experimental approach. Rather than continuing to work with from-scratch models that lack general language understanding, we pivot to investigating whether a pretrained instruction-capable model, one that already possesses general biomedical knowledge and can follow structured prompts, can be domain-adapted toward ALS-specific factual accuracy through parameter-efficient fine-tuning.

The research question shifts accordingly: given a model with existing (but domain-general) biomedical knowledge, does domain-specific fine-tuning on ALS instruction data improve factual accuracy on our benchmark, or does the limited fine-tuning data (970 pairs) degrade the model's pre-existing knowledge?

### 9.1 Methodology

We applied Quantized Low-Rank Adaptation (QLoRA) to Qwen 2.5 1.5B Instruct, a pretrained instruction-capable model with 1.5 billion parameters. The original experimental plan targeted Llama 3.2 1B Instruct, but gated access to the Llama 3.2 model was not obtained during the project timeline. Since the QLoRA fine-tuning pipeline is model-agnostic and the research question (whether domain adaptation improves factual accuracy) is independent of the specific base model, we substituted Qwen 2.5 1.5B Instruct as a comparable instruction-capable model of similar scale.

The QLoRA configuration used 4-bit NF4 (Normal Float 4) quantization for the base model weights, with LoRA adapters at rank 16, alpha 32, and dropout 0.05. The same 970 ALS instruction pairs used for the Section 8 SFT experiment served as the fine-tuning dataset, enabling direct comparison of how the same instruction data performs when applied to a pretrained instruction-capable model versus a from-scratch model. We exported the fine-tuned model to GGUF format and evaluated it against the 160-question ALS benchmark using the identical 6-stage pipeline described in Section 4, with Q8_0 as the representative quantization level.

### 9.2 Baseline evaluation

Before evaluating the QLoRA-adapted model, we established a baseline by evaluating the unmodified Qwen 2.5 1.5B Instruct model on the same 160-question benchmark. This baseline isolates the pretrained model's existing ALS knowledge and provides the reference point against which domain adaptation effects can be measured.

The unmodified Qwen 2.5 1.5B Instruct achieved 10.31% mean accuracy, 29.4% coherence, a 19.1% capability gap, and an 87.6% fabrication rate. This 10.3% accuracy exceeds every from-scratch and fine-tuned model evaluated across the entire project: the 500M from-scratch model (0.21%), the 1B from-scratch model (0.00%), the GPT-2 large fine-tuned model (3.12%), and the 1B instruction-tuned model (0.00%). A model with no ALS-specific training, relying entirely on knowledge acquired during general pretraining, outperforms all purpose-built ALS models by a substantial margin. This establishes the ceiling that QLoRA domain adaptation must beat to demonstrate value: any accuracy below 10.3% indicates that domain-specific fine-tuning degraded rather than improved the model's factual performance on ALS topics.

The failure taxonomy for the unmodified model shows 113/160 degenerate responses (70.6%), 28 confident fabrications (17.5%), 17 plausible blending instances (10.6%), and 1 accurate response (0.6%). The high degenerate rate, combined with the 10.3% accuracy concentrated in the non-degenerate minority, indicates that when the model does engage substantively with ALS questions, it draws on a meaningful reservoir of pretrained biomedical knowledge.

### 9.3 QLoRA results

Table 8 compares the unmodified Qwen 2.5 1.5B Instruct baseline against the QLoRA-adapted model on the key evaluation metrics.

**Table 8.** Qwen 2.5 1.5B Instruct baseline vs. QLoRA domain-adapted model on the 160-question ALS benchmark (Q8_0 quantization level). Domain adaptation increased coherence but decreased factual accuracy, producing a coherence-accuracy tradeoff.

| Metric                   | Qwen baseline (unmodified) | Qwen QLoRA (domain-adapted) |    Delta |
|--------------------------|:--------------------------:|:---------------------------:|---------:|
| Mean accuracy            |                     10.31% |                       7.24% |   -3.07% |
| Coherence                |                     29.4%  |                      50.0%  |  +20.6%  |
| Capability gap           |                     19.1%  |                      42.8%  |  +23.7%  |
| Fabrication rate         |                     87.6%  |                      81.0%  |   -6.64% |

The QLoRA-adapted model achieves 7.24% mean accuracy, a decrease of 3.07 percentage points from the 10.31% baseline. At the same time, coherence increases substantially from 29.4% to 50.0%, meaning that domain adaptation made the model significantly more willing to engage with ALS questions rather than producing degenerate output. However, this increased engagement comes with decreased factual accuracy, producing a coherence-accuracy tradeoff: the model sounds more knowledgeable about ALS but is actually less accurate.

The safety dimension of this tradeoff is captured by the capability gap metric. The unmodified model has a capability gap of 19.1% (29.4% coherence minus 10.3% accuracy). After QLoRA adaptation, the gap widens to 42.8% (50.0% coherence minus 7.2% accuracy). This 23.7 percentage point increase in capability gap represents a meaningful increase in ethical risk: the domain-adapted model produces coherent, confident-sounding responses about ALS that are less likely to be factually accurate than the unmodified model's responses. Users encountering the domain-adapted model would perceive it as more knowledgeable while it is actually less reliable.

The failure taxonomy shifts reinforce this interpretation. Degenerate responses decreased from 70.6% (113/160) to 50.0% (80/160), confirming the coherence improvement. However, confident fabrication increased from 17.5% (28/160) to 31.9% (51/160). The model did not learn new ALS facts from the 970 instruction pairs; instead, it learned to produce more fluent ALS-flavored text that is more likely to contain fabricated claims asserted with confidence. Plausible blending remained relatively stable at 13.8% (22/160) compared to 10.6% (17/160), and accurate responses increased marginally from 0.6% (1/160) to 1.9% (3/160).

The project's [v2 white paper](v2-white-paper.md) hypothesized that domain adaptation of a pretrained model would improve factual accuracy by directing the model's existing biomedical knowledge toward ALS-specific content. This hypothesis receives a mixed verdict: domain adaptation did shift the model's behavior toward ALS-specific engagement patterns, but at this data scale (970 instruction pairs), the shift degraded rather than improved factual accuracy. The fine-tuning appears to have partially overwritten the model's general biomedical knowledge with ALS-specific language patterns that lack the factual grounding of the pretrained knowledge they displaced.

### 9.4 Limitations

Four limitations constrain the interpretation of these QLoRA results.

**Single LoRA configuration.** Only one QLoRA configuration was tested (rank 16, alpha 32, dropout 0.05, 4-bit NF4). Alternative LoRA ranks, learning rates, or training durations might produce different accuracy-coherence tradeoffs. Higher ranks or longer training could either improve factual retention or accelerate knowledge overwriting.

**Small instruction dataset.** The same 970 instruction pairs used for the SFT experiment in Section 8 were used for QLoRA fine-tuning. This dataset may be below the threshold needed for domain adaptation to improve rather than degrade factual accuracy. Published domain adaptation work typically uses thousands to tens of thousands of instruction pairs.

**General knowledge confound.** The Qwen 2.5 1.5B Instruct model carries general biomedical knowledge from its pretraining corpus, which likely includes some ALS-related content. When the QLoRA-adapted model answers correctly, we cannot fully separate domain adaptation effects from retained pretrained knowledge. The 7.24% accuracy may reflect degraded pretrained knowledge rather than successfully adapted domain knowledge.

**Model substitution.** The original experimental plan targeted Llama 3.2 1B Instruct. Results with Qwen 2.5 1.5B Instruct may not generalize to the originally intended model, as different pretrained models encode different distributions of biomedical knowledge. However, the core finding (that 970 instruction pairs are insufficient for domain adaptation to improve accuracy) is likely model-independent, as it reflects a data-scale limitation rather than a model-specific one.

## 10. Discussion

The preceding sections established the quantitative results: a Well-fit model with near-zero factual accuracy (Section 5), a RAG comparison where even the best configuration does not exceed a no-retrieval baseline (Section 6), a fine-tuned GPT-2 large comparison revealing that pretrained knowledge improves accuracy 15x but does not resolve instruction-following limitations (Section 7), a complete SFT failure demonstrating that instruction tuning cannot create knowledge absent from the base model (Section 8), and a QLoRA domain adaptation experiment showing a coherence-accuracy tradeoff where increased domain engagement came at the cost of decreased factual accuracy (Section 9). In this section, we move beyond reporting what happened to analyzing why it happened. Each subsection centers on a causal claim about the mechanisms driving our results, drawing on established scaling laws, embedding-model theory, and the relationship between language-modeling loss and factual knowledge.

### 10.1 Data deficit and scaling laws
...
The SFT and QLoRA experiments in Sections 8 and 9 provide additional evidence that data scale is the binding constraint. Even when starting from a pretrained instruction-capable model (Qwen 2.5 1.5B Instruct) that already achieves 10.3% accuracy without any ALS-specific training, 970 domain-specific instruction pairs were insufficient to improve factual accuracy. The QLoRA fine-tuning shifted the model's behavior toward ALS-specific engagement patterns but degraded accuracy from 10.3% to 7.2%, suggesting that the instruction dataset is below the threshold needed for domain adaptation to add knowledge rather than overwrite it. This finding is consistent with the data-scaling narrative established throughout the project: at every level of the training pipeline, from pretraining to fine-tuning to domain adaptation, insufficient data produces models that learn surface patterns without acquiring factual knowledge.

### 10.2 Embedding-model impact
...
This section synthesizes results from all six evaluated models, frames data scarcity as the fundamental constraint, presents hypothesis verdicts, and identifies remaining directions for future work.

### 11.1 Synthesis

The central finding of this investigation is that data scarcity is the fundamental constraint explaining why all six model variants failed to match even the RAG baseline accuracy of 14.3%. Our ALS corpus of 142M tokens represents an 80x deficit relative to the Chinchilla-optimal ratio of approximately 20 tokens per parameter, and this deficit propagates through every approach we attempted. From-scratch models trained on this corpus establish the data floor: the 516M model achieves 0.21% accuracy despite Well-fit convergence, demonstrating that language-modeling competence does not entail factual knowledge. Scaling to 1B parameters without additional data makes this worse, not better (0.00% accuracy), confirming that the binding constraint is data volume, not model capacity. Fine-tuning a pretrained GPT-2 large on the same corpus improves accuracy 15x (to 3.12%), confirming that pretrained knowledge provides a scaffold, but the completion-based architecture produces degenerate output for 97.5% of questions, revealing that pretrained knowledge alone is insufficient without instruction-following capability. Instruction tuning the knowledge-deficient 1B from-scratch model produces catastrophic output collapse (160/160 degenerate responses), demonstrating that supervised fine-tuning cannot create knowledge that does not exist in the base model. Finally, domain adaptation of a pretrained instruction-capable model (Qwen 2.5 1.5B Instruct) via QLoRA with 970 domain-specific pairs degrades accuracy from 10.31% to 7.24%, suggesting that even when starting from a model with both knowledge and instruction-following capability, the limited fine-tuning data is insufficient to add domain knowledge and instead overwrites existing competence with surface-level domain engagement patterns.

These results converge on a single conclusion: at every level of the training pipeline, from pretraining through fine-tuning to domain adaptation, insufficient data produces models that learn surface patterns without acquiring factual knowledge. The 142M-token ALS corpus is adequate for learning how ALS research sounds but not for learning what ALS research says.

Our key findings are as follows:

1. Training at 0.25 tokens per parameter (80x below Chinchilla-optimal) is insufficient for factual knowledge acquisition in a 516M-parameter model, even when the model achieves Well-fit convergence with a validation gap of just +0.42%.
2. Language-modeling loss does not predict factual accuracy. A model can achieve excellent convergence metrics while attaining near-zero factual accuracy (0.21%), demonstrating that these are distinct capabilities with different data requirements.
3. Parameter scaling without additional data hurts rather than helps. The 1B model achieves 0.00% accuracy compared to 0.21% for the 500M model, confirming that the constraint is data volume, not model capacity.
4. Instruction tuning cannot surface knowledge absent from the base model. The 1B SFT model collapsed to 160/160 degenerate responses, producing no evaluable factual content.
5. Pretrained domain adaptation with limited data degrades accuracy. QLoRA fine-tuning of Qwen 2.5 1.5B Instruct reduced accuracy from 10.31% to 7.24% while increasing coherence, creating a capability gap where the model sounds more knowledgeable while being less accurate.
6. Fine-tuning a pretrained model improves accuracy 15x (GPT-2 large: 3.12% vs. from-scratch: 0.21%) but does not resolve instruction-following limitations inherent in completion-based architectures.
7. Domain-specific embeddings are essential for medical RAG. PubMedBERT outperforms MiniLM by 2.1x across all tested configurations, and embedding-model selection has a larger impact than chunk-size tuning.

This work makes three contributions. First, we provide a complete, reproducible, open-source pipeline for domain-specific language model training, evaluation, and comparison, spanning data collection, tokenizer training, model training on consumer hardware, GGUF export, hallucination evaluation, and RAG comparison. Second, we provide honest documentation of negative results across six model variants spanning three approach families (from-scratch training, pretrained fine-tuning, and pretrained domain adaptation), supplying empirical data points at the extreme low end of the data-scaling curve where conventional scaling laws predict failure and our results confirm it. Third, we contribute a reusable evaluation framework consisting of a 160-question curated benchmark, a 5-mode failure taxonomy, and entity-based fabrication detection using a registry of approximately 48,000 entities, applicable to other domain-specific model evaluations beyond ALS.

Table 9 presents the complete comparison across all six models evaluated in this investigation.

**Table 9.** Complete 6-model comparison on the 160-question ALS benchmark (Q8_0 quantization level). All six approaches fall short of the RAG baseline accuracy (14.3%), confirming data scarcity as the binding constraint across from-scratch training, pretrained fine-tuning, and pretrained domain adaptation.

| Model              | Approach             | Accuracy | Degenerate rate | Fabrication rate |
|--------------------|----------------------|:--------:|:---------------:|:----------------:|
| 500M from-scratch  | From-scratch         |    0.21% |           32.5% |            66.4% |
| GPT-2 large        | Pretrained fine-tune |    3.12% |           97.5% |            77.0% |
| 1B base            | From-scratch         |    0.00% |           65.0% |           100.0% |
| 1B SFT             | From-scratch SFT     |    0.00% |          100.0% |             0.0% |
| Qwen 2.5 baseline  | Pretrained instruct  |   10.31% |           70.6% |            87.6% |
| Qwen 2.5 QLoRA     | Pretrained QLoRA     |    7.24% |           50.0% |            81.0% |

*Model identifiers: 500M from-scratch = ALS-LM 516M GPT-2 Pre-LN; GPT-2 large = GPT-2 774M fine-tuned on ALS corpus; 1B base = ALS-LM 1.02B GPT-2 Pre-LN; 1B SFT = 1B base + supervised fine-tuning (970 pairs); Qwen 2.5 baseline = unmodified Qwen 2.5 1.5B Instruct; Qwen 2.5 QLoRA = Qwen 2.5 1.5B Instruct + QLoRA (970 pairs).*

The investigation was guided by four hypotheses, three defined in the project's v2 white paper (Section 4) and a fourth added during the investigation when the SFT failure motivated pivoting to pretrained instruction-capable models. Table 10 summarizes the verdict for each hypothesis.

**Table 10.** Hypothesis verdicts. H1-H3 were defined in the project's v2 white paper (Section 4). H4 was added during the investigation when the SFT failure motivated pivoting to pretrained instruction-capable models.

| Hypothesis | Description                           | Verdict  | Evidence                                                           |
|------------|---------------------------------------|----------|--------------------------------------------------------------------|
| H1         | Data quality improves accuracy        | Partial  | Corpus expanded to 153M tokens; still 80x below Chinchilla-optimal |
| H2         | Parameter scaling improves knowledge  | Rejected | 1B: 0.00% vs 500M: 0.21%; data is the constraint, not capacity     |
| H3         | Instruction tuning surfaces knowledge | Rejected | 1B SFT: 160/160 degenerate; cannot format absent knowledge         |
| H4         | Pretrained domain adaptation          | Rejected | QLoRA: 7.24% vs 10.31% baseline; 970 pairs degraded accuracy       |

The hypothesis verdicts reinforce the data scarcity thesis from different angles. The partial verdict for H1 reflects that expanding the corpus from 32MB to 676MB improved data quality and coverage but did not move the needle on factual accuracy because the expanded corpus (153M tokens) remains far below the threshold where knowledge acquisition begins. H2's rejection is particularly informative: doubling the model's capacity from 516M to 1.02B parameters while training on the same corpus produced worse results, not better, confirming that the bottleneck is data, not architecture. H3 and H4 together demonstrate that instruction tuning and domain adaptation face the same fundamental constraint. When the base model lacks factual knowledge (H3, from-scratch 1B), instruction tuning produces degenerate output because there is no knowledge to surface. When the base model has some knowledge (H4, Qwen 2.5 1.5B Instruct at 10.31%), 970 domain-specific pairs are insufficient for QLoRA to add new knowledge and instead degrade accuracy, suggesting that meaningful domain adaptation requires substantially more fine-tuning data than was available in this investigation.

### 11.2 Future work

Several directions follow naturally from our findings.

- **Instruction tuning.** The first direction identified in the original version of this paper, instruction tuning, was subsequently tested in v1.4.0 (SFT on the from-scratch 1B model) and v1.5.0 (QLoRA domain adaptation of Qwen 2.5 1.5B Instruct), with results documented in Sections 8 and 9. Both approaches confirmed that limited data (970 instruction pairs) is insufficient for instruction tuning to improve factual accuracy, leaving advanced instruction methods (larger instruction datasets, RLHF, multi-turn conversation training) as potential but increasingly constrained future work.
