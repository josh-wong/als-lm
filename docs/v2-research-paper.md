...
SFT cannot create knowledge that does not exist in the base model. This motivates a different approach: rather than attempting to teach question-answering behavior to a model that lacks foundational knowledge, we can start from a pre-trained instruction-capable model that already possesses general biomedical knowledge and adapt it toward the ALS domain.

## 9. QLoRA domain adaptation of a pre-trained instruction model

The SFT failure in Section 8 confirmed that instruction tuning requires a foundation of existing knowledge: a model trained from scratch on 153M domain-specific tokens has no internalized factual knowledge for SFT to surface, regardless of instruction dataset quality. This finding motivated a fundamentally different experimental approach. Rather than continuing to work with from-scratch models that lack general language understanding, we pivot to investigating whether a pre-trained instruction-capable model, one that already possesses general biomedical knowledge and can follow structured prompts, can be domain-adapted toward ALS-specific factual accuracy through parameter-efficient fine-tuning.
...
The central finding of this investigation is that data scarcity is the fundamental constraint explaining why all six model variants failed to match even the RAG baseline accuracy of 14.3%. Our ALS corpus of 142M tokens represents an 80x deficit relative to the Chinchilla-optimal ratio of approximately 20 tokens per parameter, and this deficit propagates through every approach we attempted. From-scratch models trained on this corpus establish the data floor: the 516M model achieves 0.21% accuracy despite Well-fit convergence, demonstrating that language-modeling competence does not entail factual knowledge. Scaling to 1B parameters without additional data makes this worse, not better (0.00% accuracy), confirming that the binding constraint is data volume, not model capacity. Fine-tuning a pre-trained GPT-2 large on the same corpus improves accuracy 15x (to 3.12%), confirming that pre-trained knowledge provides a scaffold, but the completion-based architecture produces degenerate output for 97.5% of questions, revealing that pre-trained knowledge alone is insufficient without instruction-following capability. Instruction tuning the knowledge-deficient 1B from-scratch model produces catastrophic output collapse (160/160 degenerate responses), demonstrating that supervised fine-tuning cannot create knowledge that does not exist in the base model. Finally, domain adaptation of a pre-trained instruction-capable model (Qwen 2.5 1.5B Instruct) via QLoRA with 970 domain-specific pairs degrades accuracy from 10.31% to 7.24%, suggesting that even when starting from a model with both knowledge and instruction-following capability, the limited fine-tuning data is insufficient to add domain knowledge and instead overwrites existing competence with surface-level domain engagement patterns.

These results converge on a single conclusion: at every level of the training pipeline, from pre-training through fine-tuning to domain adaptation, insufficient data produces models that learn surface patterns without acquiring factual knowledge. The 142M-token ALS corpus is adequate for learning how ALS research sounds but not for learning what ALS research says.
...
5. Pre-trained domain adaptation with limited data degrades accuracy. QLoRA fine-tuning of Qwen 2.5 1.5B Instruct reduced accuracy from 10.31% to 7.24% while increasing coherence, creating a capability gap where the model sounds more knowledgeable while being less accurate.
6. Fine-tuning a pre-trained model improves accuracy 15x (GPT-2 large: 3.12% vs. from-scratch: 0.21%) but does not resolve instruction-following limitations inherent in completion-based architectures.
...
This work makes three contributions. First, we provide a complete, reproducible, open-source pipeline for domain-specific language model training, evaluation, and comparison, spanning data collection, tokenizer training, model training on consumer hardware, GGUF export, hallucination evaluation, and RAG comparison. Second, we provide honest documentation of negative results across six model variants spanning three approach families (from-scratch training, pre-trained fine-tuning, and pre-trained domain adaptation), supplying empirical data points at the extreme low end of the data-scaling curve where conventional scaling laws predict failure and our results confirm it. Third, we contribute a reusable evaluation framework consisting of a 160-question curated benchmark, a 5-mode failure taxonomy, and entity-based fabrication detection using a registry of approximately 48,000 entities, applicable to other domain-specific model evaluations beyond ALS.

Table 9 presents the complete comparison across all six models evaluated in this investigation.

**Table 9.** Complete 6-model comparison on the 160-question ALS benchmark (Q8_0 quantization level). All six approaches fall short of the RAG baseline accuracy (14.3%), confirming data scarcity as the binding constraint across from-scratch training, pre-trained fine-tuning, and pre-trained domain adaptation.

| Model              | Approach              | Accuracy | Degenerate rate | Fabrication rate |
|--------------------|-----------------------|:--------:|:---------------:|:----------------:|
| 500M from-scratch  | From-scratch          |    0.21% |           32.5% |            66.4% |
| GPT-2 large        | Pre-trained fine-tune |    3.12% |           97.5% |            77.0% |
| 1B base            | From-scratch          |    0.00% |           65.0% |           100.0% |
| 1B SFT             | From-scratch SFT      |    0.00% |          100.0% |             0.0% |
| Qwen 2.5 baseline  | Pre-trained instruct  |   10.31% |           70.6% |            87.6% |
| Qwen 2.5 QLoRA     | Pre-trained QLoRA     |    7.24% |           50.0% |            81.0% |
...
The investigation was guided by four hypotheses, three defined in the project's v2 white paper (Section 4) and a fourth added during the investigation when the SFT failure motivated pivoting to pre-trained instruction-capable models. Table 10 summarizes the verdict for each hypothesis.

**Table 10.** Hypothesis verdicts. H1-H3 were defined in the project's v2 white paper (Section 4). H4 was added during the investigation when the SFT failure motivated pivoting to pre-trained instruction-capable models.
...
| H4         | Pre-trained domain adaptation         | Rejected | QLoRA: 7.24% vs 10.31% baseline; 970 pairs degraded accuracy         |
